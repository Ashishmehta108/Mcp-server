...
server.registerTool("ExtractDetailsFromthePrompt", {
    ...
    console.log("[Tool Called] ExtractDetailsFromthePrompt with prompt:", prompt);
    ...
    const matchedType = allTypes.find((t) => keywords.includes(t.type?.toLowerCase() || ""));
    const matchedUse = allUses.find((u) => keywords.includes(u.areaOfUse?.toLowerCase() || ""));
    if (matchedType) {
...
server.registerTool("addToCart", {
    ...
    console.log("[Tool Called] addToCart with productId:", productId, "quantity:", quantity);
    try {
...
server.registerTool("removeFromCart", {
    description: "Remove a selected product from the shopping cart. Triggered when user says 'Remove this from my cart' or clicks a button.",
    inputSchema: {
        productId: z
            .string()
            .nonempty()
    ...
    console.log("[Tool Called] removeFromCart with productId:", productId, "quantity:", quantity);
    try {
        const response = await fetch("http://localhost:3000/api/cart", {
            method: "DELETE",
            headers: {
...
server.registerTool("moreContextFromQuery", {
    description: "Get more context from a natural language query using vector db query.The llm calls it and then gets the most matching results and based upon the user context it should return the user best product based on their needs.",
    inputSchema: {
        query: z
            .string()
            .nonempty()
    ...
    try {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: "these are the most matching results of thee query from the database " +
                        JSON.stringify(res),
                },
            ],
        };
...
server.registerTool("clearCart", {
    description: "Clear the user's cart. Triggered when user says 'Clear my cart' or clicks a button.",
    inputSchema: {
...
server.registerTool("getCart", {
    ...
    const response = await axios.post("http://localhost:3000/api/cart/getcart", {
        userId,
    });
